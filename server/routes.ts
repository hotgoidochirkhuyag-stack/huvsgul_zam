...
<footer>Энэ гэрчилгээ QR кодоор баталгаажсан · Хөвсгөл зам ХХК © ${new Date().getFullYear()}</footer>
  ...
  app.patch("/api/sales/orders/:id", requireSales, async (req, res) => {
    try {
      ...
      // ── Захиалга "confirmed" болоход үйлдвэрийн төлөвлөгөө автоматаар үүснэ ──
      if (req.body.status === "confirmed" && row) {
        const productToPlant: Record<string, string> = {
          concrete_m150: "concrete", concrete_m200: "concrete",
          concrete_m300: "concrete", concrete_m400: "concrete",
          asphalt: "asphalt", asphalt_ab: "asphalt", asphalt_bb: "asphalt",
          crushed_stone: "crushing", crushed_gravel: "crushing",
        };
        const plant = productToPlant[row.product] ?? (
          row.product?.includes("concrete") ? "concrete" :
          row.product?.includes("asphalt")  ? "asphalt"  : "crushing"
        );
        const planDate = row.deliveryDate ?? new Date().toISOString().slice(0, 10);
        // Ижил захиалгын төлөвлөгөө давхцахаас сэргийлнэ
        const existing = await db.select().from(schema.productionPlans)
          .where(eq(schema.productionPlans.salesOrderId, orderId));
        if (existing.length === 0) {
          await db.insert(schema.productionPlans).values({
            date: planDate,
            plant,
            targetQty: row.quantity,
            unit: row.unit,
            notes: `Захиалга #${orderId}: ${row.customerName} — автоматаар үүссэн`,
            salesOrderId: orderId,
            autoGenerated: true,
          });
          // Мэдэгдэл: үйлдвэрийн удирдлагад мэдээлнэ
          await db.insert(schema.notifications).values({
            toRole: "ADMIN",
            title: `🏭 Үйлдвэрийн төлөвлөгөө үүслээ`,
            body: `${row.customerName} — ${row.quantity}${row.unit} ${plant === "concrete" ? "бетон" : plant === "asphalt" ? "асфальт" : "бутлуур"} · ${planDate}`,
  ...
  app.post("/api/sales/orders/:id/deduct-warehouse", requireSalesOrAdmin, async (req, res) => {
    try {
      ...
      for (const r of recipe) {
        ...
        // ── Автомат нөхөх захиалга: шинэ нөөц доод хэмжээнд хүрсэн бол ──
        const newStock = (item.currentStock ?? 0) - canDeduct;
        const minStock = item.minStock ?? 0;
        if (minStock > 0 && newStock <= minStock) {
          // Аль хэдийн хүлэн авагдаагүй pending захиалга байгаа эсэхийг шалгах
          const existingReplenish = await db.select().from(schema.warehouseOrders)
            .where(
              and(
                eq(schema.warehouseOrders.itemId, r.itemId),
                eq(schema.warehouseOrders.status, "pending"),
              )
            );
          if (existingReplenish.length === 0) {
            const criticalStock = item.criticalStock ?? 0;
            const priority = newStock <= (criticalStock > 0 ? criticalStock : minStock * 0.5) ? "critical" : "normal";
            const replenishQty = Math.max(minStock * 3, needed * 2); // 3 хоногийн норм эсвэл 2 дахин хэрэгцээ
            const expectedDate = new Date(Date.now() + 3 * 24 * 60 * 60 * 1000).toISOString().slice(0, 10); // 3 хоногийн дотор
            const [replenOrder] = await db.insert(schema.warehouseOrders).values({
              itemId: r.itemId,
              itemName: r.name,
              quantity: Math.round(replenishQty * 10) / 10,
              unit: r.unit,
              expectedDate,
              status: "pending",
              autoGenerated: true,
              triggeredBySalesOrderId: orderId,
              priority,
              note: `Автомат үүссэн — Захиалга #${orderId} (${order.customerName}) хасалтын дараа нөөц доод хэмжээнд хүрсэн. Үлдэгдэл: ${newStock.toFixed(1)}${r.unit} / Норм: ${minStock}${r.unit}`,
            }).returning();

            // WAREHOUSE мэдэгдэл
            await db.insert(schema.notifications).values({
              toRole: "WAREHOUSE",
              title: priority === "critical" ? "🚨 Яаралтай нөхөх захиалга" : "⚠️ Нөхөх захиалга үүслээ",
              body: `${r.name}: үлдэгдэл ${newStock.toFixed(1)}${r.unit} — ${priority === "critical" ? "ЯАРАЛТАЙ" : "доод хэмжээнд хүрсэн"}. Нөхөх захиалга #${replenOrder.id} автоматаар үүслээ.`,
              sourceType: "warehouse_replenish",
              sourceId: replenOrder.id,
              isRead: false,
            });
          }
        }
  ...
  // ══ Захиалгын нэгтгэл явц (агуулах + лаб + механик + үйлдвэр) ══
  app.get("/api/sales/orders/:id/progress", requireSalesOrAdmin, async (req, res) => {
    try {
      const orderId = parseInt(req.params.id);

      // 1. Үйлдвэрийн төлөвлөгөө
      const productionPlan = await db.select().from(schema.productionPlans)
        .where(eq(schema.productionPlans.salesOrderId, orderId))
        .then(rows => rows[0] ?? null);

      // 2. Лабораторийн туршилтын хүсэлт
      const labTests = await db.select().from(schema.labTestRequests)
        .where(eq(schema.labTestRequests.salesOrderId, orderId));

      // 3. Хуваарилагдсан техник
      ...
      // 4. Агуулахын хасалтын лог
      const deductions = await db.select().from(schema.warehouseDeductionLogs)
        .where(eq(schema.warehouseDeductionLogs.salesOrderId, orderId));

      res.json({ productionPlan, labTests, equipment, deductions });
    } catch (e: any) { res.status(500).json({ error: e.message }); }
  });
