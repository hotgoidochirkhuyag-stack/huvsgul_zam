        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 lg:gap-24">
          ...
          >
            ...
            <div className="space-y-6">
              {/* Үндсэн мэдээлэл: Хаяг болон Утас */}
              {[
                { icon: MapPin, title: "Хаяг", desc: "Хөвсгөл аймаг, Мөрөн сум, Хөвсгөл зам ХХК байр", link: null },
                { icon: Phone, title: "Борлуулалтын алба", desc: "+976 99112701", link: "tel:+97699112701" },
              ].map((item, idx) => (
                <div key={idx} className="flex items-start gap-6 group">
                  ...
                  <div>
                    <h4 className="text-md font-display font-bold text-foreground uppercase tracking-wide">{item.title}</h4>
                    {item.link ? (
                      <a href={item.link} className="text-primary hover:underline text-sm font-medium">{item.desc}</a>
                    ) : (
              ...
              {/* Сошиал сувгууд: 2 мөрөнд, мөр бүртээ 2 суваг */}
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                {[
                  { icon: Facebook, title: "Facebook", desc: "Хөвсгөл Зам", link: "https://www.facebook.com/h.vsg.l.zam.hhk" },
                  ...
                  { icon: Phone, title: "Viber", desc: "Чатлах", link: "viber://chat?number=+97699112701" },
                ].map((item, idx) => (
                  <div key={idx} className="flex items-start gap-4 group">
                    <div className="w-10 h-10 bg-background border border-primary/20 rounded-sm flex items-center justify-center shrink-0 group-hover:bg-primary transition-all duration-300">
                      <item.icon className="w-4 h-4 text-primary group-hover:text-primary-foreground" />
                    </div>
                    <div>
                      <h4 className="text-[11px] font-display font-bold text-foreground uppercase tracking-wider">{item.title}</h4>
                      {item.link ? (
                        <a 
                          href={item.link} 
                          target="_blank" 
